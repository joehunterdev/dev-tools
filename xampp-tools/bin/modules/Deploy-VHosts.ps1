...
Write-Host ""
Write-Host "  🌐 Deploy VHosts" -ForegroundColor Cyan
...
if ($invalidSites.Count -gt 0) {
    Write-Host "  ⚠️  Invalid sites will be skipped" -ForegroundColor Yellow
    Write-Host ""
}
...
Write-Host "    ✅ httpd-vhosts.conf ($($buildResult.Count) sites)" -ForegroundColor Green
...
Write-Host "    ✅ hosts ($($hostsResult.Count) entries)" -ForegroundColor Green
...
Write-Host "    • httpd-vhosts.conf -> $xamppRoot\apache\conf\extra\" -ForegroundColor Gray
...
Write-Host "    ✅ httpd-vhosts.conf" -ForegroundColor Green
...
if (-not $hostsDeployResult.Success) {
    Write-Host "    ⚠️  hosts - $($hostsDeployResult.Error)" -ForegroundColor Yellow
...
if ($testResult.Success) {
    ...
    Write-Host ""
    Write-Host "  ⚠️  Restoring original config..." -ForegroundColor Yellow
